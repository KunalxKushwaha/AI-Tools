...
const RemoveObject = () => {
  ...
  const onSubmitHandler = async(e)=> {
    ...
    try {
       ...
       formData.append('image', input);       formData.append('object', object)
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <textarea onChange={(e)=> setObject(e.target.value)} value={object} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 hover:border-[#65ADFF] shadow' placeholder='e.g., watch or spoon, only single object name' required />
        ...
        {
            loading? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin'></span>: <Scissors
           className='w-5'/>
    }
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        ...
        </div>
        {
          ...
          <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            ...
            <img src= {content} alt="image" className='max-w-full max-h-[500px] mt-3 object-contain rounded-md' />
          )
        }
