...
const onSubmitHandler = async(e)=> {
  ...
  try {
    ...
    const {data} = await axios.post('/api/ai/generate-article', {prompt, length:selectedLenght.length}, {
      headers: {
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form  onSubmit = {onSubmitHandler}className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e)=> setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 hover:border-[#65ADFF] shadow' placeholder='The Future of Artificial Intelligence is...' required />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            articleLenght.map((item,index)=> (
              ...
              onClick={()=> setSelectedLenght(item)}
               key={index} 
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLenght.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`}>
                {item.text}
                </span>
            ))
          }
